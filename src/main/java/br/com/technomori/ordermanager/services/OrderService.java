...
public class OrderService {
	...
	public Order insert(Order order) {
		...
		/*
		 * Order has references to OrderItem list and to Payment entities,
		 * thus this operation will be cascade to them.
		 * Therefore, it is not necessary call specific save() methods for the
		 * referenced entities.
		 */
	...
	public Order getOrderFromInsertDTO(InsertOrderDTO dto) {
		...
		/*
		 * Cross-reference between Order and Payment and between Order and OrderItem is necessary
		 * to cascade the insert operation.
		 * Payment and OrderItem entities has Foreign Key to Order, thus they must know Order entity.
		 * Order entity can cascade all operations to Payment and OrderItem, thus if these entities
		 * are referenced by Order, then it is not necessary call save() method to them.
		 * If Order does not have references to Payment and OrderItem, then it is necessary call
		 * explicitly save() methods for these entities.
		 */
		
		Order order = Order.builder()
				.customer(Customer.builder().id( dto.getCustomerId() ).build())
				.address(Address.builder().id( dto.getCustomerAddressId() ).build())
				.payment( (Payment) ctx.getBean(dto.getPaymentType()) )
				.build();
		order.getPayment().setOrder(order);
	
		List<OrderItem> orderItems = dto.getOrderItems().stream().map(
